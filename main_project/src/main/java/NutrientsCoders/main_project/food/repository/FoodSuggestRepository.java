...
public interface FoodSuggestRepository extends JpaRepository<Food, Long> {
  ...
  @Query("SELECT f FROM Food f WHERE f.breakfast = :breakfast OR f.foodCategory2 = :category" +
      " ORDER BY " +
  ...
  Page<Food> findInCategoryBreakfast(@Param("category") String category, @Param("orderbyDsce") String orderbyDsce,
                                     @Param("breakfast") String breakfast, Pageable pageable);
