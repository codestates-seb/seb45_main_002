...
const NicknameContainer = styled.div`
  ...
  align-self: flex-end;
  margin: ${style.layout.narrowMargin.height * 3}
    ${style.layout.narrowMargin.width};
`;
...
function MyPage() {
  ...
  const [imgURL, setImgURL] = useState(
    "https://media.discordapp.net/attachments/1144080831686660210/1153633184265818132/-removebg-preview.png"
  );
  ...
  function loadProfile() {
    axios
      .get("http://43.201.194.176:8080/mypage/", {
        headers: {
  ...
  function sendLeave(e) {
    e.preventDefault();
    axios
      .delete("http://43.201.194.176:8080/mypage/", {
        headers: {
          Authorization: localStorage.getItem("Authorization"),
        },
      })
            ...
            <div>
              <label>
                <input
                  ...
                  checked={user.gender === "M" ? "checked" : null}
                ></input>{" "}
                남성
              </label>
              <label>
                <input
                  ...
                  value="F"
                  onClick={(e) => setUser({ ...user, gender: e.target.value })}
                  checked={user.gender === "F" ? "checked" : null}
                ></input>{" "}
                여성
              </label>
            </div>
      ...
      {openLeave ? (
        <LeaveContainer>
          <LeaveBox>
            <h1>정말 탈퇴하시겠습니까?</h1>
            <div>
              <LeaveButton type="submit" onClick={sendLeave}>
                회원탈퇴
              </LeaveButton>
